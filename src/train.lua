...
cmd:option('-vocabfreq', 'data/vocab.freq') -- using freq list to have rare words from BITEXT!
...
cmd:option('-checkpoint_every', 100000)
...
local freqs = utils.readtxt(opt.vocabfreq)
local idx_to_token = {}
...
for i = start_i + 1, num_iterations do
  ...
  if opt.print_every > 0 and i % opt.print_every == 0 then
    ...
    local msg = 'Epoch %.2f / %d, i = %d / %d, loss = %f, ppl = %f'
    local args = {msg, float_epoch, opt.max_epochs, i, num_iterations, loss[1], math.exp(loss[1])}
...
--model:evaluate()
  ...
  --      local xv, yv = loader:nextBatch('test')
   --     xv = xv:type(dtype)
    --    yv = yv:type(dtype):view(N * T)
     --   local scores = model:forward(xv):view(N * T, -1)
     --   test_loss = test_loss + crit:forward(scores, yv)
  --  end
  --  test_loss = test_loss / num_test
  --  print('test_loss = ', test_loss)
  --  print('test_ppl = ', math.exp(test_loss))
    ----------- MOI
