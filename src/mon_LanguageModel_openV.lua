...
function LM:__init(kwargs)
  ...
  print ('vocab size idx_to_token: ' .. self.vocab_size)
    ...
    for i = 1, encoded:size(1) do
	    ...
	    local num = 0
            if current[j] ~= 0 and self.freqs[self.idx_to_token[idx]] ~= nil then -- index 0 maps to nil
                num = tonumber(self.freqs[self.idx_to_token[idx]])              
	    end
	    local num_rep = 0
            if self.freqs[self.idx_to_token[current[j]]] ~= nil then
		num_rep = tonumber(self.freqs[self.idx_to_token[current[j]]])
	    end 
            if num_rep < 1000 then
                s = s .. self.idx_to_token[current[j]] .. ':' .. num_rep .. ' '
            end
	end
        s = s .. '}\n' 
    end
